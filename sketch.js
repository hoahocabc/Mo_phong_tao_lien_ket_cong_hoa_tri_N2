...
function createUI() {
    ...
    styleButton(playButton, buttonWidth, buttonHeight);
    ...
    styleButton(spinButton, buttonWidth, buttonHeight);
    spinButton.position(buttonX, buttonY);
    spinButton.mousePressed(() => {
        isElectronSpinning = !isElectronSpinning;
        spinButton.html(isElectronSpinning ? "Tắt quay electron" : "Bật quay electron");
    });

    buttonY += 40;
    overlapButton = createButton("Bật xen phủ");
    styleButton(overlapButton, buttonWidth, buttonHeight);
    overlapButton.position(buttonX, buttonY);
    ...
    styleButton(sphereButton, buttonWidth, buttonHeight);
    sphereButton.position(buttonX, buttonY);
    ...
    styleButton(labelButton, buttonWidth, buttonHeight);
    labelButton.position(buttonX, buttonY);
    ...
    styleButton(resetButton, buttonWidth, buttonHeight);
    ...
    styleButton(instructionsButton, buttonWidth, buttonHeight, true);
    instructionsButton.position(buttonX, buttonY);
...
function styleButton(btn, w, h, isTransparent = false) {
    btn.style("width", w);
    btn.style("height", h);
...
class Atom {
    constructor(x, y, label, protons, shellCounts, electronCol) {
        ...
        for (let i = 0; i < shellCounts.length; i++) {
            ...
            // Thay đổi tốc độ quay của cả hai lớp electron
    ...
    show() {
        ...
        for (let i = 0; i < this.shells.length; i++) {
            ...
            for (let j = 0; j < this.shells[i].length; j++) {
                ...
                let spinSpeed = isElectronSpinning ? this.electronSpinSpeeds[i] : 0;
